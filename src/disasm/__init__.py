...
class Op(object):
    _regs =   dict(map(lambda x: (x.split('_')[-1].lower(),getattr(cpu,x)),filter(lambda x: x.startswith('X86_REG_') ,dir(cpu))))
    _xregs =   dict(map(lambda x: (getattr(cpu,x),x.split('_')[-1].lower()),filter(lambda x: x.startswith('X86_REG_') ,dir(cpu))))
    _xregs[0]=None
    _8bit_regs = [cpu.X86_REG_AH, cpu.X86_REG_AL, cpu.X86_REG_BH, cpu.X86_REG_BL, cpu.X86_REG_CH, cpu.X86_REG_CL, cpu.X86_REG_DH, cpu.X86_REG_DL ]
...
class C(object):
    ...
    _groups = dict(map(lambda x: (x.split('_')[-1].lower(),getattr(cpu,x)),filter(lambda x: x.startswith('X86_GRP_') ,dir(cpu))))
